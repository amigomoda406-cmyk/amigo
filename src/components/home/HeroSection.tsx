...
export default function HeroSection() {
  return (
    <section 
      className="relative flex h-[190px] md:h-[500px] border-b border-zinc-200 bg-cover bg-center bg-no-repeat"
      style={{ backgroundImage: "url('https://res.cloudinary.com/doxg77zqk/image/upload/v1785160624/Remove_basket_increase_quality_2K_202607271454.jpg')" }}
    >
      {/* Discount Badge */}
      ...
      <div className="w-[70%] pl-4 pt-6 pb-8 md:pl-16 md:pt-16 md:pb-16 flex flex-col items-start justify-center z-10 relative">
        <div className="flex items-center gap-2 mb-2 md:mb-6">
          <span className="text-[5px] md:text-sm font-bold tracking-widest text-blue-600 uppercase">New Collection</span>
          <div className="h-px w-6 md:w-16 bg-blue-600"></div>
        </div>
        <h2 className="text-[32px] md:text-[80px] font-black tracking-tighter uppercase leading-[0.85] text-zinc-900 mb-3 md:mb-8">
          Summer<br/>
        ...
        <Link href="/#categories" className="bg-[#111] text-white px-4 py-2 md:px-8 md:py-4 text-[6px] md:text-base font-bold tracking-widest uppercase flex items-center gap-1.5 md:gap-3 hover:bg-zinc-800 transition-colors">
          Shop Now <ArrowRight className="w-2 h-2 md:w-5 md:h-5" />
        </Link>
      </div>
    </section>
  );
